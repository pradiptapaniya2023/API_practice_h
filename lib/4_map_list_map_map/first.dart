...
class _FirstState extends State<First> {
  ...
  void initState() {
    ...
    mtd = MainTestData.fromJson(stringToMap);
  }
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: ListView.builder(
        itemCount: mtd!.testDataVar!.length,
        ...
        return Container(
          ...
          child: Column(
            children: [
              Text("${mtd!.testDataVar![index].id}"),
              Text("${mtd!.testDataVar![index].username}"),
              Text("${mtd!.testDataVar![index].birthDate}"),
              Image.network("${mtd!.testDataVar![index].image}")
            ],
          ),
        );
      },),
...
class MainTestData {
  List<TestData>? testDataVar ;
  int? totalVar ;
  int? skipvar ;
  int? limitVar ;

  MainTestData(this.testDataVar, this.totalVar, this.skipvar, this.limitVar);

  MainTestData.fromJson(Map<String , dynamic> json){
    // users
    testDataVar = <TestData>[];
    json["users"].forEach((e){
      testDataVar!.add(TestData.fromJson(e));
    }) ;
